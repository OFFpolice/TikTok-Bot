...
2. Настройте переменное окружения в файле `.env` с `channel_link`, `channel_id`, `bot_token`.
3. Замените ссылку на 44 строчке кода в main.py для приветственного видео сообщения.
4. Замените ссылку на 113 строчке кода в main.py для фото сообщения с предложением подписаться на Telegram канал
5. Замените идентификаторы стикеров в файле random_id.py
6. **Обязательно добавьте бота в администраторы телеграм канала.**
7. Запустите скрипт 🤗🤗🤗🤗
